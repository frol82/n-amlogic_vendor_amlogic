...
final class NativeDaemonConnector implements Runnable, Handler.Callback {
	...
	NativeDaemonConnector(INativeDaemonConnectorCallbacks callbacks, String socket,
			...
			PowerManager.WakeLock wl, Looper looper) {
		...
		try {
			...
			synchronized (mDaemonLock) {
				if (mOutputStream != null) {
					try {
						Log.i(TAG, "closing stream for " + mSocket);
						mOutputStream.close();
					} catch (IOException e) {
	...
	public NativeDaemonEvent[] executeForList(long timeoutMs, String cmd, Object... args)
		...
		final ArrayList<NativeDaemonEvent> events = new ArrayList<NativeDaemonEvent>();
		...
		do {
			...
			if (event == null) {
				Log.e(TAG, "timed-out waiting for response to " + logCmd);
				throw new NativeDaemonTimeoutException(logCmd, event);
			}
			if (VDBG) Log.i(TAG, "RMV <- {" + event + "}");
			events.add(event);
		} while (event.isClassContinue());

		final long endTime = SystemClock.elapsedRealtime();
		if (endTime - startTime > WARN_EXECUTE_DELAY_MS) {
			Log.e(TAG, "NDC Command {" + logCmd + "} took too long (" + (endTime - startTime) + "ms)");
		}
	...
	public static class Command {
		private String mCmd;
		private ArrayList<Object> mArguments = new ArrayList<Object>();
	...
	private class ResponseQueue {
			...
			synchronized (mPendingCmds) {
				...
				if (found == null) {
					...
					while (mPendingCmds.size() >= mMaxCount) {
						Log.e(TAG + "-NativeDaemonConnector.ResponseQueue",
		...
		public NativeDaemonEvent remove(int cmdNum, long timeoutMs, String logCmd) {
			...
			if (result == null) {
				Log.e(TAG + "-NativeDaemonConnector.ResponseQueue", "Timeout waiting for response");
			}
			return result;
		}
