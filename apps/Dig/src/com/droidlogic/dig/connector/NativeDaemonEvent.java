...
public class NativeDaemonEvent {
	...
	public static String[] unescapeArgs(String rawEvent) {
		...
		if (DEBUG_ROUTINE) Log.i(LOGTAG, "parsing '" + rawEvent + "'");
		...
		while (current < length) {
			...
			if (DEBUG_ROUTINE) Log.i(LOGTAG, "found '" + word + "'");
			parsed.add(word);
			...
			if (DEBUG_ROUTINE) {
				Log.i(LOGTAG, "nextSpace=" + nextSpace + ", nextQuote=" + nextQuote);
			}
			...
			if (DEBUG_ROUTINE) {
				Log.i(LOGTAG, "next loop - current=" + current +
