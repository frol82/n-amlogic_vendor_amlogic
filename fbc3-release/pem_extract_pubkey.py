...
def compute_mod_parameters(modulus):
  ...
  return {'len':wordCount, 'mod':w, 'rr':rr_words, 'n0inv':n0inv}
...
def print_header(params):
  ...
  #print "\n\tunsigned int len = %d;" % (params['len'])
  #print "\tunsigned int n[] = {%s};" % (",".join(["0x%08x" % (i) for i in params['mod']]))
  #print "\tunsigned int rr[] = {%s};" % (",".join(["0x%08x" % (i) for i in params['rr']]))
  #print "\tunsigned int n0inv = 0x%08x;" % (params['n0inv'])
